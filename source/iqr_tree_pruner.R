...
# Excluding extremely long branches based on branch lengths and root to tip distances of an unrooted tree ---------------------------------------------
#xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
...
# ape (multi2di(), ape::unroot(), drop.tip())
# caper (clade.members.list at func_nr_of_tips function)
# phytools (midpoint.root())
# ggtree (plotting the tree)
# treeio (as_tibble())
...
option_list <- list(
  make_option(c("-t", "--original_tree"), 
              ...
              help = "The input newick tree file from which the extremly long branches should be excluded.", 
              metavar = "character"),
  make_option(c("-p", "--tipprop"), 
              type = "double", default = NULL, 
              help = "The proportion of tips on a single branch that can be excluded based on the upper fence of the IQR of branch lengths. The default value is 0.05 (5%).", 
              metavar = "character"))
...
if (!interactive()) {
  ...
  opt  <- parse_args(opt_parser)
  opt$dir <- ""
  opt$original_tree <- paste0(opt$dir, "test_input/test.tre")
}
...
# write pruned tree
pruned_tree_r2t %>% ape::write.tree("pruned.tre")
...
p <- tree_tab_plotting %>% 
  ...
  geom_vline(aes(xintercept = bl_threshold, 
                 linetype = "Branch length extreme\noutliers to the right"), 
             color = "darkred") +
